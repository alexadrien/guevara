...
const changeGlobalSSL = async () => {
  const gitlabIgnoreSSL = await getEnvValue(USER_DATA_KEYS.GITLAB_IGNORE_SSL);
  if (gitlabIgnoreSSL) {
...
const getPullRequest = async (id) => {
  ...
  const pullRequest = await axios
    .get(`${gitlabAPIUrl}/merge_requests/${id}`, {
      headers: {
...
const getGitlabAPIUrl = async (projectUrl) => {
  ...
  return axios
    ...
    .then((res) =>
      res.data.filter((item) => item.path_with_namespace === projectLabel)
    )
    .then((project) => project && project[0].id)
    .then(
      (id) =>
        `${parsedUrl.protocol}//${parsedUrl.hostname}/api/v4/projects/${id}`
    );
};

const pushProject = async () => {
  await child_process.exec(`git push origin ${await getProjectActiveBranch()}`);
};
...
const createPullRequestOnGitlab = async (ticket) => {
  ...
  const gitlabToken = await getEnvValue(USER_DATA_KEYS.GITLAB_API_TOKEN);
  const pullRequest = await axios
    .post(`${apiUrl}/merge_requests`, payload, {
      headers: {
        "Private-Token": gitlabToken,
      },
    })
    .catch(
      async (err) =>
        await getPullRequest(err.response.data.message[0].split("!")[1])
    );
