...
module.exports = async () => {
  try {
    ...
    await getValueIfNotExisting(
      ...
      async () =>
        (await askUserForTrelloToken()) ||
        (await getEnvValue(USER_DATA_KEYS.TRELLO_API_KEY))
    );

    await getValueIfNotExisting(
      USER_DATA_KEYS.TRELLO_API_SECRET,
      async () =>
        (await askUserForTrelloSecret()) ||
        (await getEnvValue(USER_DATA_KEYS.TRELLO_API_SECRET))
    );
    ...
    await getValueIfNotExisting(
      ...
      async () =>
        await askUserforTheColumn("Please select the Sprint Backlog column ?")
    );
    ...
    await getValueIfNotExisting(
      ...
      async () => {
        ...
        if (userHasDailyColumn) {
          return await askUserforTheColumn(
            "Please select the Daily Backlog column ?"
          );
        }
        return CHOICES.NO;
      }
    ...
    let platform = null;
    const userHasGitlab = !!(await getEnvValue(
      USER_DATA_KEYS.GITLAB_API_TOKEN
    ));
    const userHasGithub = !!(await getEnvValue(
    ...
    switch (true) {
      case platform === COMPATIBLE_PLATFORM.GITHUB:
        ...
        if (!(await getEnvValue(USER_DATA_KEYS.GITHUB_API_PROJECT_URL))) {
          ...
          if (githubProjectUrl) {
            ...
            await writeEnvValue(
              USER_DATA_KEYS.GITHUB_API_PROJECT_URL,
              githubAPIProjectUrl
            );
          }
        }
      ...
      case platform === COMPATIBLE_PLATFORM.GITLAB:
        let gitlabUrl;
        if (!(await getEnvValue(USER_DATA_KEYS.GITLAB_IGNORE_SSL))) {
          let gitlabSsl = await askUserIfHeWantToIgnoreSSL();
          await writeEnvValue(USER_DATA_KEYS.GITLAB_IGNORE_SSL, gitlabSsl);
        }

        if (!(await getEnvValue(USER_DATA_KEYS.GITLAB_API_TOKEN))) {
          gitlabUrl = await askUserForHisGitlabProjectUrl();
          const gitlabToken =
            (await askUserToCreateAGitlabToken(gitlabUrl)) ||
            (await getEnvValue(USER_DATA_KEYS.GITLAB_API_TOKEN));
