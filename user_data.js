...
const USER_DATA_KEYS = {
  ...
  GITLAB_IGNORE_SSL: "GitlabIgnoreSSL",
...
const writeEnvFile = (content) =>
  fs.writeFileSync(ENV_FILE_NAME, content, { encoding: ENCODING });

const getEnvValue = async (valueName) => {
  try {
    const envFile = await getEnvFile();
    const valueLine = envFile
      .split(`\n`)
      .filter((line) => line.indexOf(valueName) > -1)[0];
...
${USER_DATA_KEYS.GITLAB_IGNORE_SSL}=
